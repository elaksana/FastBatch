...
class dataset:
	...
	def get_batch(self, batch_size, num_timesteps, shift_step = 1):
		try:
			...
			while words_added < words_requested:
				...
				if self.feat_list is not None: 
					unstripped_feat = unstripped_feat[:,feat_idx]
				...
				#Middle of the hdf5 file.
					...
					hdf5_filepath_mat[insertion_idx:insertion_idx + words_needed] = self.curr_hdf5_name
	...
	def _open_new_hdf5(self):
		self.hdf5.close()
		self.curr_hdf5_name = clean_line(self.file_list.readline())
		if self.curr_hdf5_name == '': return False
		self.hdf5 = h5py.File(self.curr_hdf5_name, 'r')
		self.iterator_idx = 0
		self.file_pointer = self.file_list.tell()
		return True
...
def parallel_get_batch(m):
	while(True):
		b = m['dataset'].get_batch(m['batch_size'], m['num_timesteps'], m['shift_step'])
		m['queue'].put(b)
...
class p_dataset_iterator:
	...
	def get_selected_feature_list(self):
		return search4idx(self.feat_list)[1]

	def get_batch(self):
		self.batch_lock.acquire()
		ret_tup = self.q.get()
		while ret_tup[0] is None:
			self.term_count += 1
			if self.term_count == self.k: break
			ret_tup = self.q.get()
...
if __name__ == '__main__':
	...
	ed = time.time()
	print ed - st
	print 'Went through ' + str(i) + ' batche(s).'
	sys.stdout.flush()
